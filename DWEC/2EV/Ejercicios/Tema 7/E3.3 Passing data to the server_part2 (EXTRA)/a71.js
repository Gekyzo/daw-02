...
$(document).ready(function() {
    ...
    $('#tetris_btn').on('click', function() {
        $.getJSON(
            'https://api.github.com/search/repositories?q=tetris',
            function(data) {
                let html = ''
                $.each(data.items, function(index, repository) {
                    html += '<div class="entry">'
                    html += `<h3 class="term">${repository.id}</h3>`
                    html += `<p>${repository.name} (${repository.full_name})</p>`
                    html += `<p>${repository.html_url}</p>`
                    html += '</div>'
                })
